...
func TestIntegrationApp_Run(t *testing.T) {
	testCases := []struct {
		id           string
	...
	for _, tc := range testCases {
		client := mackerelclient.NewMockClient(
			mackerelclient.MockFindAWSIntegrations(func() ([]*mackerel.AWSIntegration, error) {
				return tc.integrations.AWS, nil
			}),
		)

		for _, providers := range []string{"", "aws", "not_exist_provider"} {
			t.Run(tc.id, func(t *testing.T) {
				...
				assert.NoError(t, app.run(providers))
				if providers != "not_exist_provider" {
